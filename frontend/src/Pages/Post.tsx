...
const Post = () => {
    ...
    const headinngAttr = (content: string | undefined) => {
        if(responseData)
        {
            let h1 = content?.replace(/<h1(\s[^>]*)?>/gi, '<h1 class=" text-5xl pb-5 font-crimson font-bold">')
            let h2 = h1?.replace(/<h2(\s[^>]*)?>/gi, '<h2 class=" text-4xl font-crimson font-bold"><br>')
            let h3 = h2?.replace(/<h3(\s[^>]*)?>/gi, '<h3 class=" text-3xl font-crimson font-bold pb-5"><br>')
            let h4 = h3?.replace(/<h4(\s[^>]*)?>/gi, '<h4 class=" text-2xl font-crimson font-bold pb-5"><br>')
            let h5 = h4?.replace(/<h5(\s[^>]*)?>/gi, '<h5 class=" text-1xl font-crimson font-bold pb-5"><br>')
            let h6 = h5?.replace(/<h5(\s[^>]*)?>/gi, '<h6 class=" text-xl font-crimson font-bold pb-5"><br>')
            return h6?.replace(/<sup(\s[^>]*)?>/gi, '<sup class=" text-[#b5b5b5]">')
            ...
            <div>
                ...
                {!loading && (<div className="w-full flex justify-center items-center">
                    <div className="p-10 w-[50rem] min-h-[30rem]">
                        ...
                        <div className="md:text-[1.3rem] text-[1.2rem] font-crimson min-h-[20rem] textContent" dangerouslySetInnerHTML={{ __html: headinngAttr(responseData?.blog.content) }}></div>
                    </div>
                </div>)}
